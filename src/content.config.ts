...
const works = defineCollection({
  ...
  schema: z.object({
    title: z.string(),
    date: z.coerce.date(),
    // 単一なら category: design、複数なら category: [design, movie] のように書けます。
    // 区分の一覧は src/lib/works.ts の CATEGORIES で管理。
    category: z.union([z.string(), z.array(z.string())]).transform((v) => (Array.isArray(v) ? v : [v])),
