...
紙色の背景 × ゴシック体（Space Grotesk）× 電気ブルーのアクセントの、白基調インタラクティブなポートフォリオサイト。
作品名にホバーするとプレビュー画像がカーソルに追従して現れ、ナビはマウスに吸い付き（マグネティック）、専用カーソルが動きます。
...
`--accent`（電気ブルー）でアクセント色、`--paper` で背景の紙色、`--ink` で文字色、
`--font-display`（Space Grotesk）で見出しフォントの雰囲気を全ページ一括で変えられます。
見出しフォントを差し替えるときは `src/layouts/Base.astro` のGoogle Fonts読み込み行も合わせて変更してください。
カーソル追従・マグネティック・スクロール演出（`reveal-up` クラス）のロジックは `src/layouts/Base.astro` の `<script>` にまとまっています。
作品リストのホバー画像は、各作品の `cover` があればその画像を、無ければ色のグラデーション（`src/lib/works.ts`）を表示します。
